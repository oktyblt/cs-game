...
async function createServerContainer(options) {
  const { map, maxplayers, name, port, isOfficial, owner_id, gameMode = 'classic' } = options;
...
app.get('/api/me/server-admin', requireAuth, async (req, res) => {
  try {
    ...
    // Platform owner: nick "BrowserCS admin" → her porta (resmi dahil) AMXX _pw
    if (supabaseAdmin) {
...
const { registerVipRoutes } = require('./routes/vip');
