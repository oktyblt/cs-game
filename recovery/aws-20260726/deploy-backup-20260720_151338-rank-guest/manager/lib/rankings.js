...
/**
 * Apply kill between two registered accounts.
 * @param {number} port
...
function applyKill(port, killerUserId, victimUserId, opts = {}) {
  ...
  if (!port || !kId) return null;
  // Both sides must be registered accounts
  if (!vId || kId === vId) return null;
...
function topForPort(port, limit = 20, month = istanbulMonth()) {
  ...
  return {
    ...
    // Never expose raw UUID to public web UI unnecessarily — keep in API for debug only
    top: list.slice(0, limit).map(({ name, kills, deaths, kd }) => ({
      name,
      kills,
      deaths,
      kd,
 ...
 * Legacy v1 nickname lines are ignored (fresh season).
 */
function ingestQueueFile() {
  ...
  try {
    for (const line of raw.split('\n')) {
      ...
      // Skip legacy nickname-only lines (no UUID)
