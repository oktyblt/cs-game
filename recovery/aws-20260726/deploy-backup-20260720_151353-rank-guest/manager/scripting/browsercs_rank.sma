 ...
 * Guests can play; only registered↔registered kills count.
 */
...
public OnDeathMsg()
{
	...
	/* Only registered ↔ registered */
	if (!g_registered[killer] || !g_registered[victim])
		return;
	if (!g_userId[killer][0] || !g_userId[victim][0])
		return;
	if (equali(g_userId[killer], g_userId[victim]))
		return;
