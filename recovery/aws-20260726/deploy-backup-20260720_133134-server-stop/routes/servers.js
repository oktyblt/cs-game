...
app.get('/api/servers', async (req, res) => {
  try {
    ...
    let dbServers = [];
    if (supabaseAdmin) {
      const { data, error } = await supabaseAdmin.from('purchased_servers').select('*');
      if (error) console.warn('[api/servers] purchased_servers fetch:', error.message);
      dbServers = data || [];
    }
    ...
    async function buildServerEntry(c, dbRow) {
      ...
      const port = c.Ports?.find(p => p.PrivatePort === 27015)?.PublicPort || dbRow?.port || 0;
      if (port) seenPorts.add(port);

      let currentMap = c.Labels?.mapName || dbRow?.map || 'de_dust2';
      let currentPlayers = 0;
      const maxPlayers = parseInt(c.Labels?.maxPlayers, 10) || dbRow?.max_players || 16;
      ...
      if (port) {
        try {
          ...
          if (fs.existsSync(pwFile)) {
            hasPassword = fs.readFileSync(pwFile, 'utf8').trim().length > 0;
          }
        } catch (e) { /* sessiz */ }
      }
      ...
      return {
        ...
        hasPassword,
        status: dbRow?.status || (c.State === 'running' ? 'running' : c.State),
    ...
    for (const c of containers) {
      ...
      const dbRow = port ? portToDb.get(port) : null;
