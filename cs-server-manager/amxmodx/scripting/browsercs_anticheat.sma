...
// Soft server-side detections. Browser/WebRTC clients jitter heavily;
// teleport/speed alone must not kick — only clear firerate abuse kicks.

#define CHECK_INTERVAL 0.25
#define SPEED_GRACE 140.0
#define TELEPORT_DIST 720.0
#define WARN_COOLDOWN 12.0
#define KICK_STRIKES 10
#define SPAWN_GRACE 8.0
...
new Float:g_SpawnGraceUntil[33];
...
{
	ClearPlayer(id);
	g_SpawnGraceUntil[id] = get_gametime() + SPAWN_GRACE;
}
...
stock ClearPlayer(id)
{
	...
	g_ShotBurst[id] = 0;
	g_BurstWindowStart[id] = 0.0;
	g_SpawnGraceUntil[id] = 0.0;
}

stock bool:InSpawnGrace(id)
{
	return get_gametime() < g_SpawnGraceUntil[id];
}
...
public CheckPlayers()
{
	...
	{
		id = players[i];
		if (is_user_bot(id) || !is_user_alive(id) || InSpawnGrace(id))
			continue;
...
stock CheckSpeed(id)
{
	...
	{
		// WebRTC lag spikes look like speedhack — warn only, never kick.
		FlagPlayer(id, "speedhack", floatround(speed), false);
	}
}
...
stock CheckTeleport(id)
{
	...
	if (dist > TELEPORT_DIST && floatabs(dz) < 200.0)
	{
		...
		if (movetype != MOVETYPE_NOCLIP)
			FlagPlayer(id, "teleport", floatround(dist), false);
	}
}
...
public OnAnyPrimary_Post(weapon)
{
	...
	if ((wpn == CSW_AWP || wpn == CSW_SCOUT) && g_ShotBurst[id] >= 4)
	{
		FlagPlayer(id, "firerate", g_ShotBurst[id], true);
		g_ShotBurst[id] = 0;
	}
	else if (g_ShotBurst[id] >= 32)
	{
		FlagPlayer(id, "firerate", g_ShotBurst[id], true);
		g_ShotBurst[id] = 0;
	}
...
stock FlagPlayer(id, const reason[], value, bool:canKick)
{
	new Float:now = get_gametime();
	...
	log_amx("[BrowserCS-AC] %s (%s) %s=%d strikes=%d kickable=%d", name, auth, reason, value, g_Strikes[id], canKick ? 1 : 0);

	new msg[160];
	new reasonTr[32];
	if (equal(reason, "speedhack"))
		copy(reasonTr, charsmax(reasonTr), "hiz hilesi");
	else if (equal(reason, "teleport"))
		copy(reasonTr, charsmax(reasonTr), "isinlanma");
	else if (equal(reason, "firerate"))
		copy(reasonTr, charsmax(reasonTr), "ates hizi");
	else
		copy(reasonTr, charsmax(reasonTr), reason);

	formatex(msg, charsmax(msg), "[AC] %s: %s (%d) [%d/%d]", name, reasonTr, value, g_Strikes[id], KICK_STRIKES);
	NotifyAdmins(msg);

	if (canKick && g_Strikes[id] >= KICK_STRIKES)
	{
		g_Strikes[id] = 0;
		server_cmd("kick #%d ^"BrowserCS anti-hile^"", get_user_userid(id));
		client_print(0, print_chat, "* %s anti-hile nedeniyle atildi: %s", name, reasonTr);
	}
}
