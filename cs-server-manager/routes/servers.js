...
function registerServerRoutes(app, ctx) {
  const { docker, supabaseAdmin, queryA2S, queryA2SPlayers } = ctx;
  ...
  // Public player roster for server browser (A2S_PLAYER). :id = container id, port, or DB uuid.
  app.get('/api/servers/:id/players', async (req, res) => {
    try {
      if (typeof queryA2SPlayers !== 'function') {
        return res.status(501).json({ success: false, error: 'Oyuncu listesi desteklenmiyor', players: [] });
      }
      ...
      const [info, detail] = await Promise.all([
        ...
        queryA2SPlayers('127.0.0.1', meta.port).catch(() => null),
      ]);
      ...
      const players = raw
        .map((p, i) => {
          ...
          const dur = Number.isFinite(p.duration) && p.duration >= 0 ? Math.round(p.duration) : null;
          return {
            index: i,
