/**
 * BrowserCS — halflife.wad + map CRC join fix (additive, live-safe)
 */
(function () {
  'use strict';
  ...
  async function buildHalflifeResponse() {
    ...
    building = (async () => {
      ...
      if (!r1.ok || !r2.ok) throw new Error('halflife1/2 indirilemedi');
      ...
      if (!isWad3(out) || out.length < 4e6) throw new Error('birleşik halflife.wad geçersiz');
      try { console.log('[bcs-halflife-fix] ✓ halflife.wad', (out.length / 1048576).toFixed(1) + 'MB'); } catch (_) {}
