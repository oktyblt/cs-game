/**
 * CS Web Game API — Standalone Cloudflare Worker
 * Bağımsız Worker olarak çalışır, Pages'dan bağımsız.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/relay') {
      ...
      if (!room) { room = { host: null, clients: new Set() }; globalThis.CS_SOCKETS.set(roomId, room); }
      if (role === 'host') {
        room.host = server;
        server.addEventListener('message', e => { for (const c of room.clients) { try { c.send(e.data); } catch(_) { room.clients.delete(c); } } });
        server.addEventListener('close', () => { room.host = null; globalThis.CS_ROOMS.delete(roomId); for (const c of room.clients) { try { c.close(); } catch(_) {} } });
      } else {
        room.clients.add(server);
        const rd = globalThis.CS_ROOMS.get(roomId); if (rd) rd.players = Math.min(rd.maxplayers, (rd.players||1)+1);
        server.addEventListener('message', e => { if (room.host) { try { room.host.send(e.data); } catch(_) { room.host=null; } } });
        server.addEventListener('close', () => { room.clients.delete(server); const r=globalThis.CS_ROOMS.get(roomId); if(r&&r.players>1) r.players--; });
      }
